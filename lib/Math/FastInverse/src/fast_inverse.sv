...
module fast_inverse #(
    ...
    input logic [2*DATAWIDTH-1:0] A,
    input logic A_dv,

    output logic [FRACBITS-1:0] A_inv,
    output logic A_inv_dv
    );

    localparam unsigned MAX_SHIFT = $clog2(2*DATAWIDTH)+1;
    ...
    integer i;
    logic signed [$clog2(2*DATAWIDTH):0] w_shift_amt;
    logic signed [$clog2(2*DATAWIDTH):0] r_shift_amt;
    always_comb begin
        w_shift_amt = 0;
        for (i = DATAWIDTH + FRACBITS-1; i >= 0; i = i - 1) begin
            if (r_A[i] == 1'b1) begin
                w_shift_amt = MAX_SHIFT'(i - 2 * FRACBITS + 1);
                break;
            end
        end
    end

    logic [2*DATAWIDTH-1:0] two_fp = 2 << (2 * FRACBITS);
    ...
    always_comb begin
        AX = ((r_A_scaled * X) >> (2 * FRACBITS));
        two_fp_minus_AX = (two_fp - AX);
        X_two_AX = X * two_fp_minus_AX;
        w_X_next = (X_two_AX >> (2 * FRACBITS));
    end
    ...
    always_ff @(posedge clk) begin
        if (~rstn) begin
            ...
            case (current_state)
                IDLE: begin
                    if (A_dv) begin
                        // r_A <= {{(DATAWIDTH-FRACBITS){A[DATAWIDTH-1]}}, A, {FRACBITS{1'b0}}};
                        r_A <= A;
                    end
                ...
                POSTPROCESS: begin
                    A_inv <= w_A_inv[2*FRACBITS - 1:FRACBITS];
                    A_inv_dv <= 1;
                end
